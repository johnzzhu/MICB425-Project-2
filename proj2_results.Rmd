...
Geochemical data from cruise 72 (???) was loaded in order to draw correlations with norC abundance.
...
ggplot(gene_abd_depth, aes(x = Type, y = Depth_m)) +
  ...
  scale_y_reverse(lim=c(200,100)) +
  labs(title = "Abundance of norC (DNA & RNA) across depths", x = NULL, y = NULL) + 
  theme(legend.position="none", plot.title=element_text(size=12))

```

**Figure ???**  
Sum of FPKM abundance (value labels) of norC at the gemone and expression level across different depths (Y-axis, meters).    



The abundance of norC differs with depth. No norC gene was found at either the gemone or expression level above 100m. For DNA the abundance is increasing with depth until 150m, where it is maximal. Then the abundance abruptly drops at 165m, and then is relatively high again at 200m. For RNA, norC abundance is relatively low up to depth 135m, only slowly increasing. Then the abundance is notably greater at 150m, and maximum RNA abundance is reached at 165m. Then RNA abundance decreases again at 200m.  
...
# Change NAs to "unclassified" at the level you want to plot
  ...
  mutate(Genus = ifelse(Genus == "environmental samples", Family, Genus)) %>% # John: use Family classification for Genus classifications that say "environmental samples". I think it's more informatibe 
  mutate(Abundance = ifelse(Abundance == 0, NA, Abundance)) %>% 
...
**Figure ???**
Sum of FPKM abundance of norC stratified at the genus level. Abundance is shown at both the gemone and expression level across different depths (Y-axis, meters). The size of each circle is proportional to abundance.     
...
plot_abd_depth = ggplot(gene_abd_depth, aes(x = Type, y = Depth_m)) +
  ...
  scale_y_reverse(lim=c(200,10)) +
  labs(x = "", y = NULL) + 
  theme(legend.position="none", plot.margin = unit(c(0,0,0.55,0), "cm"))
...
Plotting abundance vs N species concentration
...
Correlating RNA abundance with NO3, N2O & NH4
