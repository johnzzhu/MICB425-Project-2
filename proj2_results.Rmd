...
Geochemical data from cruise 72 (???) were loaded in order to draw correlations with norC abundance.
...
ggplot(gene_abd_depth, aes(x = Type, y = Depth_m)) +
  ...
  scale_y_reverse(lim=c(200,10)) +
  labs(caption = "Figure ??? FPKM abundance (value labels) of norC \nat the genome and expression level across different \ndepths.", x = NULL) + 
...
#   geom_point(aes(size = ifelse(Abundance == 0, NA, Abundance), color = Type), position = position_dodge(0.6)) +
#   scale_y_reverse(lim=c(200,10)) +
#   labs(title = "Abundance of norC (DNA & RNA) by genera across depths", x = NULL, y = NULL) + 
#   theme(axis.text.x = element_text(angle = 20, hjust = 1), legend.position="none", plot.title=element_text(size=12))
...
# Make 0 values not show up, keep points from overlapping
  geom_point(aes(size = Abundance, color = Type), position = position_dodge(0.6)) +
  scale_y_reverse(lim=c(200,10)) +
  labs(caption = "Figure ??? FPKM abundance of norC stratified at the class level. Abundance is \nshown at both the genome and expression level across different depths. The size of \neach circle is proportional to abundance.", x = "Class") + 
  theme(axis.text.x = element_text(angle = 20, hjust = 1), plot.caption = element_text(size = 12, hjust = 0),plot.margin= margin(0,0,0,20))
```
...
The relation between norC abundance and nitrogen species was examined.

``` {r fig.width=12, fig.height=8}

gene_abd_depth_meta = metadata %>% 
  select(Depth_m, NH4_uM, N2O_nM, NO2_uM, NO3_uM) %>% 
...
ggplot(gene_abd_depth_meta, aes(x = uM, y=Abundance)) +
  geom_point() +
  facet_wrap(~Type + Nutrient, scales="free") +
  geom_smooth(method='lm') +
  labs(caption = "Figure ??? FPKM abundance of norC at different nitrogen species concentrations. Abundance is shown at both the genome and expression level.") + 
...
RNA_abundance = data.frame("_" = numeric(0), "_"= numeric(0),"_"= numeric(0),"_"= numeric(0))
RNA_abundance <- rbind(RNA_abundance, nh4_lm$coefficients['NH4_uM',])
RNA_abundance <- rbind(RNA_abundance, n2o_lm$coefficients['N2O_nM',])
RNA_abundance <- rbind(RNA_abundance, no3_lm$coefficients['NO3_uM',])
rownames(RNA_abundance) <- (c("NH4_uM", "N2O_uM","NO3_uM"))
colnames(RNA_abundance) <- (c("Estimate", "Std. Error","t value","Pr(>|t|) (p-value)"))
kable(RNA_abundance,caption="Table ??? Correlation Data of norC RNA Abundance Across Nitrogen Species Concentrations.")
